...
- **2026-08-14(P50/P99 瓶颈定位:MySQL 双 fsync 是根因)**:报告 3/4 遗留"P50 70-195ms、P99 174-5819ms 瓶颈在哪"专项定位。**方法**:新增 PerfStats 消费链 8 阶段埋点(环形缓冲 + 每 5s 打 PERF 行,零客户端改动,`im.perf.stats.enabled` 可关)→ 单变量隔离实验(只改 MySQL flush 配置)。**根因证实**:双 fsync(flush=1+sync_binlog=1)钉死 insert 服务时间 10.8ms——隔离实验 flush=2+0 后 insert **10.8→2.7ms**、客户端 P50 198→99ms、P99 **5819→381ms**(-93%)、积压消失;**放大器** = asyncExecutor 无界队列(饱和时 async_wait p50 822ms,只堆不拒);**800/s 硬拐点 = fsync 服务时间×16 线程的容量边界**(flush=2 后容量上移 ×2+,无拐点)。**500/s 全链拆账**(flush=2,52ms):cli_to_broker 3 + mq_broker 1 + **mq_wakeup 20(最大单项,RocketMQ 拉取唤醒)** + 保序段 1.5 + 排队 ~19 + insert 2.6 + ack 5 ≈ 52ms ✓。**保序的真相**:保序段仅 1.5ms,不是延迟主体;代价在并行度上限(8 路)+ 队列连坐。**决策**:MySQL 已恢复 1+1,800/s 写为已知容量边界(面试讲法:知道根因、知道怎么改、为什么没改)。详见 docs/延迟瓶颈定位.md。顺带修复 reset-data.sh 删 54 万用户 IN 列表超 shell 参数上限(改子查询)。
- **2026-08-14(下行可靠投递:服务端发件箱 + 补拉对账回执)**:补上"下行推送尽力而为"的最后一块可靠性拼图。**背景**:在线丢推(背压跳过/MQ 丢/回执丢)时接收方位置已越过丢失 seq,重连补拉够不到 = 永久丢消息(旧链路缺口)。**方案**:业界标准**未确认队列(outbox)**——推过就入箱(`im:push:outbox` zset,score=下次检查时间)、DELIVER_ACK 到达才出箱、扫描器 5s 一轮 ZPOPMIN 原子认领(chat 多实例不双发)超时重推(退避 30s/1m/2m/5m,上限 5 次 + 7 天兜底)。**关键语义**:接收方离线时【不删不计数】等上线重推(删了补拉也够不到);已 DELIVERED 自动清理;重推"至少一次"+ 客户端 serverMsgId 去重 = 恰好一次效果。**配套**:补拉回来的单聊消息也回 DELIVER_ACK(对账回执,离线漏推标已送达);connect 已盖章 receiverId → chat 校验回执来源防伪造(防 A 伪造 B 的回执压掉重推)。**验证**:单测 12 个全过;verify-outbox.js 两场景实测 PASS(在线丢推→同 serverMsgId 重推→回 ACK→A 见已送达→出箱无第三次推送;离线漏推→补拉→对账回执→A 见已送达)。详见 docs/下行可靠投递.md。
- **2026-08-14(全系统设计审计:双线排查出 6 个高危)**:发件箱落地后主动做安全面 + 可靠性面审计,每条结论落 file:line 证据。**🔴 安全侧 3 个可主动攻击**:① 群管理接口零授权(addMember/removeMember 不校验调用者,任何登录用户可拉人/踢人,踢人连 userId 都不从鉴权上下文取);② 跨会话消息注入(conversationId 客户端可控 + 单聊无归属校验——把 conversationId 设成 `B#C` 即可向任意两人私聊投毒,拉取侧校验只防读不防写);③ 群消息发送无 isMember 校验(`g_` 路由信客户端前缀,非成员刷垃圾消息)。**🔴 可靠性侧 3 个丢消息窗口**:① 取号后落库前崩溃(位点已提交 + dedup 键 7 天压制重传 = dedup 补落缺口实锤,当前最大真实缺口,方案已设计);② seq 空洞无检测(位置越过后漏的消息永久不可见,单聊靠 outbox 兜住、群聊没有);③ 群消息三重洞(无 outbox + 漏推不自愈 + 幂等重复分支无 ACK → 用户手动重发造成真重复)。**🟡 新发现**:outbox 背压跳过被误计数(重推全被跳过仍计次 → ~13.7 分钟后放弃)、seq 计数器可被 allkeys-lru 驱逐(不只重启)、雪花 ID 多实例 1/32 撞主键(workerId=PID hash mod 32,容器 PID=1 必撞,撞后 DuplicateKeyException 被当幂等吞、日志不可区分)、登录/注册无限流、SHA-256+8hex salt 弱哈希、token 明文、握手不绑定 deviceId、8MB 帧上限、listMembers 泄露成员。修复路线 P0(dedup 补落 + 服务端重算 conversationId + 客户端 gap 检测)→ P1(群授权/seq 惰性校准/workerId 显式化)→ P2(群 outbox/异步化/限流哈希)。详见 docs/设计审计2026-08.md。
- **2026-08-13(六指标系统性压测,报告4)**:im-loadtest 重构(接收方统计/端到端延迟/空连接 ramp/群扇出)+ 观测工具(observe-lt.sh/bench-route.sh)+ 全矩阵压测。**① 端到端 P99**:健康区 174-182ms(推送段仅 ~5ms,ACK 落库是主要成分)。**② 空连接**:5k/10k/20k/30k/40k/50k/60k 七档单进程全握手 100%、0 掉线——**60k 是客户端同机单进程硬顶**(本地端口池 64,511 个);双进程 100k(每节点 50k)稳态零掉线、connect RSS<850MB、服务端上限未触及;120k 档掉线 44% 根因是客户端 GC 停顿(8 核打满→心跳延迟→服务端 30s idle 兜底),非服务端故障。**③ 路由**:GET 单连 23k/s/pipeline 1.08M/s,路由本身非瓶颈;群扇出 2×(S-1) 次串行 Redis 是优化空间。**④ 送达/不重**:窗口口径 99.6-100%(尾截断),不重率恒 1.0000,乱序/缺号 O(1);DB 对账 100%。**⑤ TPS 拐点**:健康区 <600/s(P99≤212ms),拐点 600-800/s(P99→1.4s),**硬拐点 800/s 断崖**;崩溃区不丢消息(connect 0 失败/MQ 0 积压/重试 0)。**⑥ 扇出**:10→200 人 ackP99 26→63ms 近线性,扇出倍数精确=成员-1(4 万人投递 0 重复),多群并行无共享瓶颈;带群已读 77 倍放大(100 人 3106ms)——"按需查询"读放大是群已读在活跃大群的真实成本。**数据层**:慢查询 0 条(崩溃区延迟全是排队);dedup key 92 万占 Redis 98%(7 天 TTL 远超分钟级幂等窗口);im_message 索引 15.6MB>数据 11.5MB;无大 value。**方法学**:慢日志 TABLE 模式拖慢 P99 4 倍必须用 FILE;reset/切换后首轮失真 ~1.7s 须预热轮;多档连跑后偶发 1s 失真须重跑验证。报告见 docs/压测报告4.md。
...
- [docs/延迟瓶颈定位.md](docs/延迟瓶颈定位.md) — **P50/P99 瓶颈定位:MySQL 双 fsync 是根因**(消费链 8 阶段埋点 + 隔离实验 + 500/s 全链拆账 + 800/s 拐点机制 + 保序的真相 + 优化方向 + 生产配置决策)
- [docs/下行可靠投递.md](docs/下行可靠投递.md) — **下行可靠投递:服务端发件箱(未确认队列)+ 补拉对账回执**(在线丢推死结 / outbox 结构与分支语义 / 离线不删不计数 / ZPOPMIN 多实例安全 / 防伪造 / 与补拉兜底的分工 + 验证)
- [docs/设计审计2026-08.md](docs/设计审计2026-08.md) — **设计审计 2026-08:当前还有什么漏洞**(双线排查:安全面 3 高危(群管理零授权/跨会话注入/群消息无成员校验)+ 可靠性面 3 高危(落库前崩溃窗口/seq 空洞/群三重洞)+ 10 中危 + 8 低危 + P0/P1/P2 修复路线 + 面试讲法)
