...
- **多端踢人 + 心跳修复(2026-08-09)**:同端类型单设备模式 + 手动踢设备。**KICK 用 Redis Pub/Sub**(im:kick 频道):chat `convertAndSend` 发布 {userId, deviceId},所有 connect 节点订阅(KickSubscriber),**各节点本地判目标**(ChannelManager.get != null 才关)——广播 + 本地判目标,只有持有连接者关;即发即弃的缺点被"删 token 让重连被拒"兜底。**同端踢人**:登录时 `kickSameTypeDevices(userId, deviceType, excludeDeviceId)` 踢同 deviceType 旧设备(删旧 token + publish KICK,排除正在登录的设备);**手动踢**:`POST /api/auth/devices/{deviceId}/kick`。**踢人四步**:删 token(踢死根基)→ publish KICK → connect 在目标 EventLoop 上 `ChannelManager.remove + close`(close 触发 onDisconnect 清 Redis 会话表)。**坑**:KickSubscriber 不能在 close 前 clear ConnectionContext——否则 onDisconnect 时 userId 为 null 提前返回,不清 Redis 会话表(会话残留,设备列表误判在线)。**心跳修复**:HeartbeatHandler 续期前查 `sessionRegistry.getNode`,路由表没了(被踢)→ 拒续期 + 关连接——否则被踢的死连接靠心跳续命,TTL 兜底失效。
- **信令与消息分通道(2026-08-09)**:MQ 上下行各拆信令通道,消息与信令**队列级隔离**(信令积压/重试不影响消息投递)。**上行**:消息走 `client2server`,信令(DELIVER_ACK/READ_ACK)合并走 `client2signal`——connect 的 `dispatchDeliverAck`/`dispatchReadAck` 发到 client2signal;chat 的 DeliverAckConsumer/ReadAckConsumer 都订阅 client2signal,靠字段区分(DELIVER 有 serverMsgId,READ 有 untilSeq,各自跳过对方)。**下行**:消息走 `server2client`,信令(ACK-STORE/DELIVER/READ/GROUP_READ)走 `server2signal`——chat `DownstreamProducer.topicFor(contentType)` 按信封类型分流(MSG→消息,其余→信令);connect 用两个 `DownstreamConsumer` 实例分别消费(构造函数参数化 topic + group 后缀,`im-connect-consumer-{nodeId}-msg`/`-signal`)。**配置**:application.yml `rocketmq.topics.signalUpstream/signalDownstream`。KICK 走 Redis Pub/Sub(不在 MQ)。
