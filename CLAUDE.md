...
- 核心认知:**EventLoop 只做收发,阻塞调用丢业务线程池**(Java 17 无虚拟线程)。Netty 出站写必须用 `channel.writeAndFlush`(ctx.write 会绕过末尾 encoder)。**已知例外(待修)**:HandshakeHandler 握手 authenticate、HeartbeatHandler 续期、信令转发(MessageDispatcher 的 MQ send)仍在 EventLoop 线程上做同步 Redis/MQ 调用——原则与代码现状有落差,面试被追需如实说"已识别、未异步化"。
