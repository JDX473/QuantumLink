...
- **分支工作流**:新功能在 `dev` 分支开发 → 验证通过后 merge 到 `main`。当前在 `dev`,最新完成 信令与消息分通道 + 多端踢人(2026-08-09)。
...
- **身份体系(服务端分配为主)**:user_id(注册分配)、device_id(客户端首启持久生成 `d_+16hex`,重装不变,多端/设备管理基础)、server_msg_id(落库雪花生成,消息正式身份)。幂等键 client_msg_id 客户端生成(**UUID** `crypto.randomUUID()`),去重键 = sender_id+client_msg_id。下行带 server_msg_id+seq,不带 client_msg_id。**登录注册**:`POST /api/auth/register` + `POST /api/auth/login`(SHA-256+salt),登录返回 token/deviceId/userId,存 Redis `im:token:` 供 connect 握手校验。
- **三层身份模型**:username(登录名)全局唯一(DB 唯一索引+注册查重),用于登录/搜索/发消息唯一解析——**已实现**。nickname(昵称)/remark(好友备注)**尚未实现,是演进方向**:im_user 目前无 nickname 列、无 im_friendship 表(schema.sql 未建),演进路径为 im_user 加 nickname → 建 im_friendship(含 remark)→ 展示按 remark→nickname→username 取。**userId 是不变锚点**,username/nickname/remark 只影响显示名,不影响业务结构。
- **水平扩展(无 gateway,客户端直连)**:多 connect 节点各自订阅自己的 MQ tag(nodeId = `host:port`);Redis 会话表 `im:session:{userId}:{deviceId} → nodeId`;chat 发下行时查会话表定位节点 → 打 nodeId tag 投 server2client(消息)/server2signal(信令) → 只有目标节点消费。调度接口 `GET /api/connects`(最少连接决策,见下条,已替代旧的静态 connect.nodes 列表 + 随机直连)。nodeId 必须与会话表值、MQ tag、调度接口地址三处一致。**nodeId 构造(2026-08-13)**:ConnectConfig 的 `host` 字段(`im.connect.host`/`IM_CONNECT_HOST`,start-all.sh 透传,默认 127.0.0.1),`nodeId() = host:port`——跨机器部署(压测/桌面端在另一台电脑)必须设客户端可达的地址(云上=公网 IP),nodeId 三处一致靠"单一构造点"自动成立。**坑(自动探测被否)**:曾实现"自动探测本机对外 IP"(UDP 路由探测),但云服务器探测到的是内网 IP(如 172.26.x.x),公网客户端依旧连不上——云部署必须显式设公网 IP,不自动探测。**注意**:杀 java 进程会连带杀掉 RocketMQ namesrv/broker(它们也是 java),须用 `scripts/start-middleware.cmd` 恢复。
...
- **单测与脚本(2026-08-05 起,随功能持续增长)**:**单测 139 个全过**(im-common 28 / im-connect 15 / im-chat 96,JUnit5 + Mockito),`mvn test` 全过;JaCoCo 绑定 3 个模块 pom。**范围**:纯逻辑/业务类全覆盖(工具/编解码/服务层/Controller),**不测**纯 DTO/实体/基础设施(Netty 服务器/RocketMQ 生产消费者/MinIO/启动类)——测了是测框架 mock。业务类指令覆盖 83%。**快速启动脚本**:`scripts/start-all.cmd`(服务端一键)+ `scripts/start-client.cmd`(客户端一键)。**cmd 脚本硬编码坑**(全踩过):① 编码必须 GBK/纯 ASCII(UTF-8 被 cmd 按 GBK 误读成乱码命令);② Electron 启动必须带 app 路径 `.`(否则 "To run a local app" 提示);③ `findstr` 正则管道里不可靠,简化 `findstr ":port"`;④ `if...() 多行块` 偶发解析异常,用 `goto` 跳转;⑤ `start` 标题必须带引号;⑥ bash 调 cmd 的引号会坏,须独立 cmd 进程验证。**验证脚本(2026-08-13 起全部走真实鉴权)**:`scripts/verify-*.js` + `clients/verify-*.js` 共 21 个回归脚本,全部用 `clients/test-lib.js`(newUser 随机用户名注册登录)拿真 token 握手——**禁止再写硬编码 test-token**(旧"预置 Redis token"假设与现行鉴权不兼容,握手 AUTH_FAILED)。client-core `_pullConversation` 已带 Bearer 头(历史接口需鉴权)。
...
- **多端踢人 + 心跳修复(2026-08-09)**:同端类型单设备模式 + 手动踢设备。**KICK 用 Redis Pub/Sub**(im:kick 频道):chat `convertAndSend` 发布 {userId, deviceId},所有 connect 节点订阅(KickSubscriber),**各节点本地判目标**(ChannelManager.get != null 才关)——广播 + 本地判目标,只有持有连接者关;即发即弃的缺点被"删 token 让重连被拒"兜底。**同端踢人**:登录时 `kickSameTypeDevices(userId, deviceType, excludeDeviceId)` 踢同 deviceType 旧设备(删旧 token + publish KICK,排除正在登录的设备);**手动踢**:`POST /api/auth/devices/{deviceId}/kick`。**踢人四步(解耦:路由表主动删,不依赖 close 成功)**:① 删 token(踢死根基)→ ② publish KICK → ③ connect 在目标 EventLoop 上**主动删 Redis 路由表(sessionRegistry.remove)+ 删本地 ChannelManager** → ④ close(**close 失败重试一次**,回调里 `isActive()` 判断;仍失败靠心跳兜底——路由表已删,续期前 getNode=null → 关连接)。**为什么前置删路由表**:删路由表不再依赖 close 成功触发 onDisconnect——close 失败则 onDisconnect 不跑、路由表残留,死连接靠心跳(getNode 查得到)续命,兜不住;主动先删,close 成不成都由心跳 ≤10s 揪出。**坑**:KickSubscriber 不能在 close 前 clear ConnectionContext——否则 onDisconnect 时 userId 为 null 提前返回,清理失效(路由表虽已主动删,仍保语义干净)。**心跳修复**:HeartbeatHandler 续期前查 `sessionRegistry.getNode`,路由表没了(被踢)→ 拒续期 + 关连接——否则被踢的死连接靠心跳续命,TTL 兜底失效。
- **异步鉴权 + 待鉴权帧队列(2026-08-09)**:握手 authenticate 从"EventLoop 上同步 Redis GET"改为 **Lettuce async**(`SessionRegistry.authenticateAsync`,RedisFuture→CompletableFuture),不阻塞 EventLoop——Redis 抖动不再卡住同 EventLoop 上的所有连接(原"最该改"的 🔴 bug,已修)。**待鉴权帧队列**:Netty channelRead 连续,鉴权异步期间到达的业务帧(MSG/DELIVER_ACK/READ_ACK)入 **per-connection ArrayDeque** 暂存(**有界 64 帧**,超限=恶意灌帧直接关);鉴权成功回投 EventLoop 后**按序补处理**(不丢不乱序),失败清队列关连接;**超时 3s 兜底**(Redis 挂/慢不死等)。**关键点**:① 异步回调必须 `channel.eventLoop().execute` 回投(回调线程不能碰 channel);② completeAuth **先 bind(authenticated=true)再补处理 pending**,MessageHandler 才放行;③ pending 只在鉴权期间暂存,PING 由更前的 HeartbeatHandler 消化不进来。**验证**:`scripts/verify-async-auth.js`(raw TCP 握手+MSG 连发,MSG 入队补处理 → 回 ACK-STORE + 对方收到)+ verify-kick 回归 + 139 单测全过。
- **信令与消息分通道(2026-08-09)**:MQ 上下行各拆信令通道,消息与信令**队列级隔离**(信令积压/重试不影响消息投递)。**上行**:消息走 `client2server`,信令(DELIVER_ACK/READ_ACK)合并走 `client2signal`——connect 的 `dispatchDeliverAck`/`dispatchReadAck` 发到 client2signal;chat 的 DeliverAckConsumer/ReadAckConsumer 都订阅 client2signal,靠字段区分(DELIVER 有 serverMsgId,READ 有 untilSeq,各自跳过对方)。**下行**:消息走 `server2client`,信令(ACK-STORE/DELIVER/READ/GROUP_READ)走 `server2signal`——chat `DownstreamProducer.topicFor(contentType)` 按信封类型分流(MSG→消息,其余→信令);connect 用两个 `DownstreamConsumer` 实例分别消费(构造函数参数化 topic + group 后缀,`im-connect-consumer-{nodeId}-msg`/`-signal`)。**配置**:application.yml `rocketmq.topics.signalUpstream/signalDownstream`。**坑(topic 配置与实现不一致)**:chat 的 DeliverAckConsumer/ReadAckConsumer 硬编码 `client2signal`,未用 @Value 读 yml → `signalUpstream` 是死配置;connect 侧全部 topic(server2client/server2signal/client2server/client2signal)硬编码在代码里(NettyConnectServer/UpstreamProducer/MessageDispatcher),connect 无配置文件 → 两侧 topic 一致性靠人工保证。KICK 走 Redis Pub/Sub(不在 MQ)。
...
- **可靠投递**:双 ACK——**STORE**(chat 落库,可靠锚点)+ **DELIVER**(接收方客户端回 DELIVER_ACK 帧 → connect 转发 client2signal → chat 更新状态 SENT→DELIVERED → 回 DELIVER 给发送方)。发送方超时同一 client_msg_id 重传(3s 超时、指数退避、上限 6 次)。
- **下行发件箱 outbox(2026-08-14,服务端未确认队列)**:补"下行推丢"最后一跳——在线丢推(背压跳过/MQ 丢/回执丢)时接收方位置已越过丢失 seq,重连补拉够不到,必须服务端重推。**结构**:Redis zset `im:push:outbox`(member=serverMsgId,score=下次检查时间)+ hash `im:push:outbox:meta`(attempts,bornMs)。**生命周期**:单聊 push 真发出 MQ 才入箱(离线不推不入箱,补拉兜底);DELIVER_ACK 到达出箱(实时推/补拉都算);扫描器 5s 一轮 **ZPOPMIN 原子认领**(chat 多实例不双发)到期成员:已 DELIVERED/消息没了 → 清理;**接收方离线 → 不删不计数**(位置可能已越过,删了补拉也够不到,等上线重推;对方补拉回的对账回执会自然清掉);在线 → 重建 payload 重推(退避 30s/1m/2m/5m,上限 5 次放弃 + 7 天兜底,ERROR 日志)。**至少一次语义**:客户端按 serverMsgId 去重渲染。**配套**:补拉回来的单聊消息也回 DELIVER_ACK(对账回执,离线漏推标已送达);connect 已盖章 `receiverId`=连接认证用户 → DeliverAckConsumer 校验 `message.receiverId==ack.receiverId` 防伪造(防 A 伪造 B 回执压掉重推);群消息不回 DELIVER_ACK(无 per-member DELIVER 语义,chat 丢弃)。`sendEnvelope` 改返回 boolean(真发 MQ 才算)。验证:verify-outbox.js 两场景 PASS + OutboxServiceTest 12 个。**遗留**:dedup 补落缺口(SETNX 过但 insert 前崩 → 重传被挡 7 天,方案已设计未实现)——2026-08-14 设计审计升级为 🔴 R1(P0 修复项)。详见 docs/下行可靠投递.md、docs/设计审计2026-08.md。
...
- **MVP 范围(历史定义,已全部完成并超出)**:原定 单聊/单节点/无网关/无群聊/无多端/有 token 鉴权/必须有 RocketMQ。后续已补:群聊、单聊+群聊已读、多端+踢人、水平扩展(多节点+最少连接调度)、压测、信令分通道。
- **架构**:im-common / im-connect(port 19001/19002,Netty,避开 Windows 动态端口;默认 9999) / im-chat(port 8081,Spring Boot) / im-loadtest。connect 与 chat 零代码依赖,只经 MQ+Redis 通信。
- **下行统一信封 DownstreamEnvelope**:`{type, to, deviceId, targets, data}`——投递元数据(type/to/deviceId/targets)与内容(data)分离,connect 只解析顶层、不懂业务内容;data 用嵌套对象(非 JSON 字符串)避免双重转义。**type**:ACK(回执)/ MSG(消息)/ READ(已读事件)/ GROUP_READ(群已读计数);to 非空 = 单播,targets 非空 = 群播(按节点聚合的成员列表,与 to 互斥);deviceId 空 = 多端全推。消息(MSG)走 `server2client`,信令走 `server2signal`。
...
- 核心认知:**EventLoop 只做收发,阻塞调用丢业务线程池**(Java 17 无虚拟线程)。Netty 出站写必须用 `channel.writeAndFlush`(ctx.write 会绕过末尾 encoder)。**已知例外(待修)**:HeartbeatHandler 续期、信令转发(MessageDispatcher 的 MQ send)仍在 EventLoop 线程上做同步 Redis/MQ 调用——原则与代码现状有落差,面试被追需如实说"已识别、未异步化"(握手 authenticate 已异步化,见下条)。
...
- `docs/幂等与取号原子性.md` — 幂等与取号原子性(SETNX 一步原子 + Orderly 串行 + DB 唯一索引)
...
- `docs/多端踢人.md` — 多端踢人(Redis Pub/Sub KICK + 删 token + 同端单设备 + 手动踢 + 心跳修复)
- `docs/信令分通道.md` — 信令与消息分通道(队列级隔离 + topic 布局 + 配置不一致坑)
...
- `docs/设计审计2026-08.md` — **设计审计 2026-08:当前还有什么漏洞**(双线排查结论:安全 3 高危[群管理零授权/跨会话注入/群消息无成员校验] + 可靠性 3 高危[落库前崩溃窗口=dedup 补落实锤/seq 空洞/群三重洞] + 10 中危 + 8 低危 + P0/P1/P2 修复路线)——修复时逐项更新本文
