...
- **水平扩展(无 gateway,客户端直连)**:多 connect 节点各自订阅自己的 MQ tag(nodeId = `host:port`);Redis 会话表 `im:session:{userId}:{deviceId} → nodeId`;chat 发下行时查会话表定位节点 → 打 nodeId tag 投 server2client(消息)/server2signal(信令) → 只有目标节点消费。调度接口 `GET /api/connects`(最少连接决策,见下条,已替代旧的静态 connect.nodes 列表 + 随机直连)。nodeId 必须与会话表值、MQ tag、调度接口地址三处一致。**注意**:杀 java 进程会连带杀掉 RocketMQ namesrv/broker(它们也是 java),须用 `scripts/start-middleware.cmd` 恢复。
