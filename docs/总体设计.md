...
IM 的标准身份模型分三层,当前项目已实现"登录名",昵称/备注为演进方向:

| 层 | 唯一性 | 谁设置 | 作用 | 当前状态 |
|----|--------|--------|------|---------|
| **username**(登录名/账号) | **全局唯一**(DB 唯一索引 + 注册查重) | 注册时定 | 登录、搜索加好友、发消息时唯一解析到人 | ✅ 已实现 |
| **nickname**(昵称) | 可重复 | 用户自己 | 对外展示的名字,可随时改 | ⬜ 演进方向 |
| **remark**(好友备注) | 可重复 | **好友关系里 A 给 B 设的** | 只对 A 显示,别人看不到 | ⬜ 演进方向 |

- **username 必须全局唯一**:发消息填用户名要能唯一确定人(`/api/users/resolve?username=` 用 selectOne)。若未来支持用户名可重复,需引入 Discord 式 `username#tag` 或微信式独立账号体系。
- **userId 是不变身份锚点**:消息/会话/设备/好友关系都绑 userId;username/nickname/remark 只影响"显示名",不影响任何业务结构。
- **演进路径**:① `im_user` 加 `nickname` 字段(可空,默认=username);② 建 `im_friendship(from_user_id, to_user_id, remark)` 好友关系表;③ 会话列表/消息展示时按 `remark → nickname → username` 取显示名。已有结构无需改动。
