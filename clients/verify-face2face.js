...
(async () => {
  // 随机 4 位码:避开 Redis im:f2f:{code} 的 5 分钟 TTL 残留窗口(固定码会偶发撞上次的群)
  ...
  // 1. A 建群
  const ra = await face2face(a, code);
  console.log('[A] 输入 ' + code + ':', JSON.stringify(ra));
  if (!ra.isNewGroup) { console.error('❌ 应新建群'); process.exit(1); }
  const gid = ra.groupId;
  console.log('✅ A 新建群:', gid, ra.name);

  // 2. B/C 并发加入同一群
  const [rb, rc] = await Promise.all([face2face(b, code), face2face(c, code)]);
  ...
  // 3. 重复输入幂等
  const rb2 = await face2face(b, code);
  ...
  const cc = new ImClient({ host: process.env.IM_CONNECT_HOST || '127.0.0.1', port:19002, token:c.token, deviceId:c.deviceId, deviceType:'desktop', quiet:true,
