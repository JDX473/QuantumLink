...
(async () => {
  const a = await newUser('offA');
  const b = await newUser('offB');
        ...
        B = new ImClient({
          ...
          handlers: {
            onConnected: () => {
              ...
              // 阶段1:B 在线,A 发 2 条
              setTimeout(() => {
                clientA.sendMessage({ receiverId: b.userId, msgType: 'TEXT', content: '在线-1', clientTime: Date.now() });
                clientA.sendMessage({ receiverId: b.userId, msgType: 'TEXT', content: '在线-2', clientTime: Date.now() });
                log('A 已发 2 条(在线)');
                // 阶段2:2s 后 B 断开(模拟离线)
                setTimeout(() => {
                  log('B 断开(模拟离线)...');
                  ...
                  // 阶段3:A 离线期间发 3 条
                  setTimeout(() => {
                    clientA.sendMessage({ receiverId: b.userId, msgType: 'TEXT', content: '离线-1', clientTime: Date.now() });
                    clientA.sendMessage({ receiverId: b.userId, msgType: 'TEXT', content: '离线-2', clientTime: Date.now() });
                    clientA.sendMessage({ receiverId: b.userId, msgType: 'TEXT', content: '离线-3', clientTime: Date.now() });
                    log('A 已发 3 条(离线期间)');
    ...
    const newB = new ImClient({
      ...
      handlers: {
        onConnected: () => {
          ...
          // 会话 id 约定:min(a,b)#max(a,b)(ConversationIdUtil 排序拼接,与方向无关)
          const convKey = [a.userId, b.userId].sort().join('#');
          newB.conversationLastSeq.set(convKey, 2);
          newB._pullOffline();
        },
        onMessage: (m) => {
          received.push(m);
